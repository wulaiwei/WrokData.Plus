...
namespace WorkData.EntityFramework.UnitOfWorks
{
    ...
    {
        TDbContext GetCurrentDbContext<TDbContext>(Dictionary<string, DbContext> dic)
            where TDbContext : DbContext;

        TDbContext GetCurrentDbContext<TDbContext>(Dictionary<string, DbContext> dic, string conString)
            where TDbContext : DbContext;
